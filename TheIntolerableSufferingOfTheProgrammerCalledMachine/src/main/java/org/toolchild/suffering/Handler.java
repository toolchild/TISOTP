...
public class Handler {
  ...
  public void render(Graphics2D graphics2d, int lastSecondTicks, int lastSecondFrames) {
    ...
    graphics2d.drawImage(spriteManager.backgroundSheet.getSpriteSheet(), 0, 0, levelWidth * 64, levelHeight * 64, null);
  ...
  public void createLevel() {
    levelWidth = spriteManager.getLevelImage().getWidth();
    levelHeight = spriteManager.getLevelImage().getHeight();
    for (int y = 0; y < levelHeight; y++) {
      for (int x = 0; x < levelWidth; x++) {
        int pixel = spriteManager.getLevelImage().getRGB(x, y);
        int red = (pixel >> 16) & 0xff;
        int green = (pixel >> 8) & 0xff;
        int blue = (pixel) & 0xff;
        int size = 64;
        if (red == 0 && green == 0 && blue == 255) addPlayer(new Player(x * size, y * size, size, size, Id.player, this, spriteManager.getPlayer()));
        if (red == 0 && green == 0 && blue == 0) addTile(new Grass(x * size, y * size, size, size, true, Id.wall, this, spriteManager.getGrass()));
        if (red == 255 && green == 255 && blue == 0) addTile(new PowerUpBlock(x * size, y * size, size, size, true, Id.powerUpBlock, this, spriteManager.powerUpBlock));
        if (red == 66 && green == 66 && blue == 66) addEntity(new Mob1(x * size, y * size, size, size, Id.mob1, this, spriteManager.getMob1()));
        if (red == 255 && green == 0 && blue == 0) addEntity(new BlueCrystal(x * size, y * size, size, size, Id.blueCrystal, this, spriteManager.getBlueCrystal()));
      }
    }
  }
